...
const cardVariants = cva("absolute will-change-transform", {
  variants: {
    variant: {
      dark: "flex size-full flex-col items-center justify-center gap-6 border border-white/12 bg-[oklch(0.105_0.018_181/.88)] p-6 text-white backdrop-blur-md",
      light:
        "flex size-full flex-col items-center justify-center gap-6 border border-white/12 bg-white/[0.045] p-6 text-white backdrop-blur-md",
    },
  },
  defaultVariants: {
    variant: "dark",
...
>(
  (
    ...
    const range = React.useMemo<[number, number]>(() => [start, end], [start, end]);
    ...
    const dy = useTransform(scrollYProgress, rotateRange, [4, 16]);
    const blur = useTransform(scrollYProgress, rotateRange, [2, 28]);
    const alpha = useTransform(scrollYProgress, rotateRange, [0.18, 0.28]);
    ...
    return (
      <motion.div
        ...
        style={{
          ...
          filter: variant === "light" ? lightFilter : "none",
          ...style,
        }}
